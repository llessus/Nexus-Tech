...
export const ComentarAutorPage = () => {
  ...
  return (
    <div className="container" style={{ maxWidth: '700px', margin: '0 auto', padding: '2rem 0' }}>
      <Card style={{ padding: '1.5rem', marginBottom: '2rem', display: 'flex', gap: '1rem', alignItems: 'center' }}>
        ...
        <div><h3 style={{ fontSize: '1rem' }}>{noticia.titulo}</h3><p style={{ fontSize: '0.813rem', color: 'var(--text-muted)' }}>Por {autor?.nome}</p></div>
      </Card>
      {!enviado ? (
        <Card style={{ padding: '2rem' }}>
          ...
          <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '1rem', textAlign: 'right' }}>{texto.length}/500</p>
          <Button onClick={() => setEnviado(true)} disabled={!texto.trim()}>Enviar Comentário</Button>
        </Card>
      ) : (
        <Card style={{ padding: '2rem', textAlign: 'center' }}>
          <p style={{ color: 'var(--color-success)', fontSize: '1.25rem', fontWeight: 600, marginBottom: '1rem' }}>✓ Comentário enviado!</p>
